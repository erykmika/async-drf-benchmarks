"""
Views for Variant A (Asynchronous)
Using ADRF AsyncViewSets for asynchronous operations
"""
...
# Sample data for CPU-bound endpoints (same as variant_s)
...
class BenchmarkViewSet(AsyncViewSet):
    """Async ViewSet for benchmark endpoints"""

    http_client = HTTPClient("A")
    cache_client = CacheClient("A")

    @action(detail=True, methods=["get"], url_path=r"articles/(?P<article_id>\d+)")
    async def get_article(self, request, article_id=None):
    ...
    async def update_article(self, request, article_id=None):
    ...
    async def external_call(self, request):
        """IO-3: Simulate external HTTP call with 100ms delay"""
        await asyncio.sleep(0.1)  # 100ms delay
        return Response({
            "status": "success",
            "data": {"external": "data", "timestamp": asyncio.get_event_loop().time()}
        })

    @action(detail=True, methods=["get"], url_path=r"cache/(?P<article_id>\d+)")
    async def cache_or_db(self, request, article_id=None):
        """IO-4: Get from cache or database"""
        cache_key = f"article:{article_id}"
        cached_data = await self.cache_client.aget(cache_key)
        ...
        if cached_data:
            ...
            await self.cache_client.aset(cache_key, json.dumps(data), ex=3600)

        return Response(data)

    @action(detail=False, methods=["post"], url_path="queue")
    async def queue_task(self, request):
    ...
    async def list_small(self, request):
    ...
    async def list_large(self, request):
    ...
    async def mixed_db_analyze(self, request, article_id=None):
    ...
    async def mixed_http_serialize(self, request):
    ...
    async def mixed_pipeline(self, request, article_id=None):
        """MIX-3: Cache -> DB -> process -> cache"""
        cache_key = f"stats:{article_id}"
        cached_stats = await self.cache_client.aget(cache_key)
        ...
        if cached_stats:
            ...
            stats = analyze_text(article.content)
            await self.cache_client.aset(cache_key, json.dumps(stats), ex=3600)
            source = "computed"
    ...
    async def health(self, request):
        return Response({"status": "healthy", "variant": "a"})
